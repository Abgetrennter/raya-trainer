...
function Get-ForbiddenEntryReason {
    ...
    if ($leafName.Equals("Ra3Trainer.settings.json", [StringComparison]::OrdinalIgnoreCase)) {
        return "legacy user settings file Ra3Trainer.settings.json must not be distributed"
    }

    if ($leafName.Equals("Ra3Trainer.Agent.dll", [StringComparison]::OrdinalIgnoreCase)) {
        return "legacy Agent DLL Ra3Trainer.Agent.dll must not be distributed"
    }

    if ($leafName.Equals("code.txt", [StringComparison]::OrdinalIgnoreCase)) {
        return "code.txt must not be distributed"
    }
...
foreach ($path in $ZipPath) {
    ...
    try {
        foreach ($entry in $archive.Entries) {
            ...
            if ($leafName.Equals("agent-release.json", [StringComparison]::OrdinalIgnoreCase)) {
                $hasAgentManifest = $true
                $agentManifestContent = Read-ZipTextEntry $entry
            }
        ...
        # Phase 6 provenance gate: every packaged Agent DLL must ship with agent-release.json
        # and the manifest's sha256/architecture must match the actual binary bytes.
        if ($hasAgentDll -and -not $hasAgentManifest) {
            $violations.Add("$($resolvedPath.Path): missing agent-release.json beside RayaTrainer.Agent.dll")
        }
        elseif ($hasAgentDll -and $hasAgentManifest) {
            $manifest = $null
            try {
                $manifest = $agentManifestContent | ConvertFrom-Json
            }
            catch {
                $violations.Add("$($resolvedPath.Path): agent-release.json is not valid JSON")
            }

            if ($null -ne $manifest) {
                $sha = [System.Security.Cryptography.SHA256]::Create()
                try {
                    $actualHash = ([System.BitConverter]::ToString($sha.ComputeHash($agentDllBytes))).Replace("-", "").ToLowerInvariant()
                }
                finally {
                    $sha.Dispose()
                }
                ...
                if ([string]::IsNullOrWhiteSpace($manifestHash) -or
                    -not $manifestHash.ToLowerInvariant().Equals($actualHash, [StringComparison]::Ordinal)) {
                    $violations.Add("$($resolvedPath.Path): agent-release.json sha256 does not match RayaTrainer.Agent.dll")
                }

                $manifestArch = [string]$manifest.architecture
                if (-not "x86".Equals($manifestArch, [StringComparison]::OrdinalIgnoreCase)) {
                    $violations.Add("$($resolvedPath.Path): agent-release.json architecture must be x86; actual '$manifestArch'")
                }

                # Signature state is informational, not a release gate. The signed field
                # is recorded honestly in agent-release.json (true only when an Authenticode
                # signer verified); the SHA-256 match above is the tamper-evidence gate
                # (ADR 0031 §3: "没有签名证书时 SHA-256 清单仍是阻塞发布的最低门禁").
            }
        }
    }
    finally {
