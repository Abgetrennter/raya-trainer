...
.DESCRIPTION
    ...
    path in the v6 manifest tail (sources region; field@44 = region size,
...
$nsm.AddNamespace('a', 'uri:ea.com:eala:asset')
$stripped = 0
foreach ($node in @($xml.SelectNodes('//a:GameObject', $nsm))) {
    [void]$node.ParentNode.RemoveChild($node)
    $stripped++
}
foreach ($inc in @($xml.SelectNodes('//a:Include', $nsm))) {
    if (@('DATA:Worldbuilder.xml', 'DATA:Japan/Units_SinglePlayerCampaign/JapanMechaKing.xml') -contains $inc.GetAttribute('source')) {
...
$xml.Save($staged7)
Write-Host "  stripped $stripped GameObject node(s) + art includes"
...
Write-Host "  v6 session id: $sessionHex"
