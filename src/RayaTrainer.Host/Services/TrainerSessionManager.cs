...
public sealed partial class TrainerSessionManager : ITrainerSessionService, ITrainerDiagnosticsSource, IProductControlSessionHost, IReinforcementProjectionPublisher, ISecretProtocolProjectionPublisher, IDisposable
{
    ...
    public async Task<AttachResult> AttachTargetAsync(TrainerManifest manifest, TrainerTarget target)
    {
        if (_phase is not (SessionPhase.Detached or SessionPhase.Offline))
        {
            throw new InvalidOperationException("会话正在连接中，不能重复附加。");
        }

        _phase = SessionPhase.Attaching;
        _manifest = manifest;
        ...
        if (agentInstalledHooks)
        {
            ...
            await TryEnableOverlayAsync().ConfigureAwait(false);
            ...
            await TryRequestRuntimeAssetLoadAsync().ConfigureAwait(false);
        }
        ...
        _phase = agentInstalledHooks ? SessionPhase.PatchesInstalled : SessionPhase.Attached;
    ...
    public async Task<SessionInstallOutcome> InstallPatchesAsync(TrainerManifest manifest, string diagnosticsDir)
    {
        if (_phase is not (SessionPhase.Attached or SessionPhase.PatchesInstalled))
        {
        ...
        _diagnosticState.CaptureRuntimeState(_featureController, _arePatchesInstalled);
        await TryEnableOverlayAsync().ConfigureAwait(false);
        _phase = SessionPhase.PatchesInstalled;
        return new SessionInstallOutcome(agentInstallResult, CreatePatchInstalledStatus(agentInstallResult));
    }

    public Task ResetPatchesStateAsync()
    {
        if (_phase == SessionPhase.Attaching)
        {
            throw new InvalidOperationException("会话正在连接中，不能恢复 Patch。");
        }

        if (_phase is not (SessionPhase.Attached or SessionPhase.PatchesInstalled))
        {
            // Idempotent: nothing to restore when the session is already cleared.
            return Task.CompletedTask;
        }

        return ClearSessionStateAsync(restoreAgentPatches: true);
    }

    public async Task MarkTargetOfflineAsync()
    {
        if (_phase == SessionPhase.Attaching)
        {
            throw new InvalidOperationException("会话正在连接中，不能结束会话。");
        }

        await ClearSessionStateAsync(restoreAgentPatches: false).ConfigureAwait(false);
        _phase = SessionPhase.Offline;
    }

    private async Task ClearSessionStateAsync(bool restoreAgentPatches)
    {
        ...
        {
            await TryStopOverlayAsync().ConfigureAwait(false);
            try
            {
                await _agentBackend.RestorePatchesAsync(TimeSpan.FromSeconds(5)).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
    ...
    public void Dispose()
    {
        ...
        // (ResetPatchesStateAsync) asks the Agent to restore.
        // restore=false performs no awaits, so the clear completes synchronously here.
        _ = ClearSessionStateAsync(restoreAgentPatches: false);
    }
    ...
    public FeatureCapabilitySnapshot GetFeatureCapability(TrainerFeature feature)
    {
        ...
        var unavailableReason = _unavailableFeatureReasons.GetValueOrDefault(feature.RawName);
        var agentStatus = _diagnosticState.AgentStatus;
        var nativeCapabilities = (NativeRuntimeCapabilities)(agentStatus?.NativeRuntimeCapabilities ?? 0u);
        var layeredInstall = nativeCapabilities.HasFlag(NativeRuntimeCapabilities.LayeredHookInstall);
        var optionalHooksReady = nativeCapabilities.HasFlag(NativeRuntimeCapabilities.OptionalHookSetReady);
        var behavior = TrainerFeatureBehaviorCatalog.TryGetBehavior(feature.RawName);
        var usesOptionalHookSet = layeredInstall && !optionalHooksReady &&
            behavior is not null &&
            (behavior.Kind == TrainerFeatureBehaviorKind.NativePulse ||
             (behavior.Kind == TrainerFeatureBehaviorKind.NativeToggle &&
              behavior.AsNativeToggle()?.HookId != 41));
        if (usesOptionalHookSet && string.IsNullOrEmpty(unavailableReason))
        {
            unavailableReason = "不可用：Agent 可选 Hook 批次安装失败，核心运行时已降级保留。";
        }

        IReadOnlyCollection<uint> installedHooks = Array.Empty<uint>();
        if (agentStatus is { InstalledHookCount: > 0 })
        {
            installedHooks = layeredInstall && !optionalHooksReady
                ? new uint[] { 41 }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (_arePatchesInstalled && _agentBackend is not null && ShouldEnableOverlay(_agentTarget))
            {
                ...
                {
                    // Diagnostics refresh was cancelled (session offline / re-attach);
                    // this is not an overlay fault and must not surface as degraded.
                }
                catch (Exception ex)
                {
        ...
        {
            // Diagnostics refresh cancelled (session offline / re-attach): the caller
            // teardown path owns the state transition; nothing to record here.
        }
        catch (Exception ex)
        {
            _diagnosticState.RecordEvent(
    ...
    private async Task TryRequestRuntimeAssetLoadAsync()
    {
        ...
        try
        {
            ...
            var result = await _agentBackend
    ...
    private async Task TryEnableOverlayAsync()
    {
        ...
        try
        {
            ...
            var status = await _agentBackend
    ...
    private async Task TryStopOverlayAsync()
    {
        if (_agentBackend is null || !ShouldEnableOverlay(_agentTarget))
        {
            return;
    ...
    // (low-level keyboard hook), which works under RA3's in-match DirectInput. The returned
    // task carries the pipe round-trip; hotkey callers discard it so the UI thread never blocks.
    public Task ToggleOverlayVisibilityAsync()
    {
        var backend = _agentBackend;
        ...
        return ApplyOverlayVisibilityAsync(backend, next);
    }
