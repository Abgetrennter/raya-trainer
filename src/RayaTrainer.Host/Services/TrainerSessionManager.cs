...
public sealed partial class TrainerSessionManager : ITrainerSessionService, ITrainerDiagnosticsSource, IProductControlSessionHost, IReinforcementProjectionPublisher, ISecretProtocolProjectionPublisher, IDisposable
{
    ...
    {
        // 2026-08-19 实机确认：独占全屏下 attach 会破坏 D3D9 显示状态，游戏退化为
        // topmost 合成窗口且无法再 Alt+Tab 离开（锁定源已二分收敛到 overlay 栈，
        // 禁用整条 overlay 后全屏切换恢复正常）。在根因修复前整条 overlay 栈禁用：
        // 探针设备、EndScene/Reset detour、WndProc 子类化均不安装，F10 与诊断读取
        // 同步门控。窗口化路径不受影响，但为避免误触一并停用；恢复时删除此门禁。
        return false;
#pragma warning disable CS0162 // unreachable code below the suspension gate
        if (target is null)
        {
            return false;
        }
