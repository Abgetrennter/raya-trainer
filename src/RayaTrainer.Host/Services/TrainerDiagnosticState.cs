...
internal sealed class TrainerDiagnosticState
{
    ...
    private TrainerDiagnosticFailure? _lastDiagnosticFailure;
    private RuntimeReadState _runtimeReadState = RuntimeReadState.Empty;
    ...
    // Event-driven stage coloring (design doc §6.2): failures recorded with a stage
    // attribution escalate the matching diagnostics stage beyond its snapshot-derived state.
    private readonly Dictionary<string, DiagnosticStageState> _eventStageOverrides = new(StringComparer.Ordinal);
    private readonly object _stageOverrideSync = new();
    // Session-scoped correlation sequence (design doc §4.4): every recorded failure carries
    // an operation id so the status text, event stream, export bundle and crash log can be
    // matched from a single screenshot. Never reset — ids stay unique for the whole session;
    // the product plane reuses wire IntentIds instead.
    ...
    /// <summary>
    /// Structured last failure captured via <see cref="RecordFailure"/> (design doc §6.1
    /// single entry). Null until the first failure of the current attach; carries the
    /// unified vocabulary fields for the diagnostics page and the export bundle.
    /// </summary>
    public TrainerDiagnosticFailure? LastDiagnosticFailure => _lastDiagnosticFailure;
    ...
    public void ResetForAttach()
    {
        ...
        _runtimeReadState = RuntimeReadState.Empty;
    ...
    public void CaptureAgent(InjectedAgentBackend? backend)
    {
        ...
        if (_agentStatus is AgentStatusPayload status)
        {
            RecordEvent(
                ...
                DiagnosticEventCodes.AgentHandshake,
        ...
        if (_latestMismatchDiagnostic is not null && result.SkippedHooks.Count > 0)
        {
            var (code, message) = _latestMismatchDiagnostic.Kind switch
            {
                MismatchKind.RuntimePatchSet =>
                    (DiagnosticEventCodes.AgentPatchsetInstallMismatch,
                     $"PatchSet {_latestMismatchDiagnostic.SubjectId} 入口字节不匹配。"),
                MismatchKind.PatchSetIpConflict =>
                    (DiagnosticEventCodes.AgentPatchsetCodeflowIpConflict,
                     $"PatchSet {_latestMismatchDiagnostic.SubjectId} 线程 IP 冲突。"),
                _ => (DiagnosticEventCodes.AgentHookMismatch,
                      $"Hook #{_latestMismatchDiagnostic.SubjectId} @ 0x{_latestMismatchDiagnostic.HookAddress:X8} 字节不匹配。")
            };
            RecordEvent(DiagnosticEventSeverity.Warning, code, message, result.ReportPath, TrainerErrorVocabulary.StagePatch);
        }
        else
        {
            RecordEvent(
                result.SkippedHooks.Count == 0 ? DiagnosticEventSeverity.Info : DiagnosticEventSeverity.Warning,
                result.SkippedHooks.Count == 0 ? DiagnosticEventCodes.PatchInstalled : DiagnosticEventCodes.PatchInstalledPartial,
    ...
    public void CaptureRuntimeState(ITrainerFeatureController? controller, bool arePatchesInstalled)
    {
        ...
        var previous = _runtimeReadState;
        try
        {
            var gameMode = controller.ReadGameMode();
            var gameThreadTick = controller.ReadGameThreadTick();
            _runtimeReadState = new RuntimeReadState(
                gameMode,
                gameThreadTick,
                Attempted: true,
                Succeeded: true,
            ...
            if (previous.Failed)
            {
                RecordEvent(
                    DiagnosticEventSeverity.Info,
                    DiagnosticEventCodes.RuntimeRefreshRecovered,
                    "运行时诊断读取已恢复。");
            }
        }
        catch (Exception ex)
    ...
    public void CaptureOverlayStatus(AgentOverlayStatusPayload status)
    {
        ...
        if (status.IsReady)
        {
            ClearStageOverride(TrainerErrorVocabulary.StageGame);
        }
        OnChanged?.Invoke();
    }
    ...
    public void CaptureOverlayFailure(string message, AgentOverlayStatusPayload? status = null)
    {
        _overlay = status is AgentOverlayStatusPayload payload
            ...
            : new OverlayDiagnosticSnapshot(
                ...
                message);
        RecordEvent(DiagnosticEventSeverity.Warning, DiagnosticEventCodes.OverlayDegraded, message, stage: TrainerErrorVocabulary.StageGame);
    }
    ...
    {
        RecordStageOverride(severity, stage);
        if (_events.Add(severity, code, message, detail, operationId))
        {
            OnChanged?.Invoke();
        }
    }
    ...
    /// <summary>
    /// Session-scoped monotonic correlation id (design doc §4.4). Callers that already hold
    /// a wire correlation value (product plane IntentId) pass it explicitly instead.
    /// </summary>
    public long NextOperationId() => Interlocked.Increment(ref _nextOperationId);

    /// <summary>
    /// The single entry for errors reaching the diagnostics page (design doc §5 rule 5).
    /// Callers must pass the unified vocabulary code; domain/stage/operationId/evidence
    /// should be supplied whenever known so the failure stays locatable end to end.
    /// </summary>
    ...
    {
        var correlationId = operationId ?? NextOperationId();
        _lastDiagnosticErrorCode = code;
        _lastDiagnosticError = message;
        _lastDiagnosticFailure = new TrainerDiagnosticFailure(
            code, message, domain, retryHint, stage, correlationId, evidence, detail);
        RecordEvent(DiagnosticEventSeverity.Error, code, message, detail, stage, correlationId);
    }

    /// <summary>
    /// Event-driven stage coloring (design doc §6.2): escalates the UI stage matching the
    /// failure attribution. Vocabulary stages that have no UI stage are resolved first
    /// (signature → profile); product-plane layered stages surface on the product control
    /// card and never color the stage strip.
    /// </summary>
    private void RecordStageOverride(DiagnosticEventSeverity severity, string? stage)
    {
        if (severity is not (DiagnosticEventSeverity.Warning or DiagnosticEventSeverity.Error))
        {
            return;
        }

        var uiStage = ResolveUiStage(stage);
        if (uiStage is null)
        {
            return;
        }

        var escalated = severity == DiagnosticEventSeverity.Error
            ? DiagnosticStageState.Error
            : DiagnosticStageState.Warning;
        lock (_stageOverrideSync)
        {
            if (!_eventStageOverrides.TryGetValue(uiStage, out var current) || StageRank(escalated) > StageRank(current))
            {
                _eventStageOverrides[uiStage] = escalated;
            }
        }
    }

    private void ClearStageOverride(string stage)
    {
        lock (_stageOverrideSync)
        {
            _eventStageOverrides.Remove(stage);
        }
    }

    private void ClearStageOverrides()
    {
        lock (_stageOverrideSync)
        {
            _eventStageOverrides.Clear();
        }
    }

    private static string? ResolveUiStage(string? stage) => stage switch
    {
        null => null,
        "" => null,
        TrainerErrorVocabulary.StageSignature => "profile",
        TrainerErrorVocabulary.StageAdmission or
        TrainerErrorVocabulary.StageExecution or
        TrainerErrorVocabulary.StageEffect or
        TrainerErrorVocabulary.StageCompensation => null,
        _ => stage,
    };

    private static int StageRank(DiagnosticStageState state) => state switch
    {
        DiagnosticStageState.Error => 2,
        DiagnosticStageState.Warning => 1,
        _ => 0,
    };
        ...
        int maxEvents = TrainerDiagnosticEventBuffer.Capacity)
    {
        ...
        // Capture the immutable runtime-read generation once. Background refresh may publish the
        // next generation concurrently, but one UI snapshot can never mix its flags and values.
        var runtimeRead = _runtimeReadState;
        var game = new GameRuntimeDiagnosticSnapshot(
            runtimeRead.GameMode,
            FormatGameMode(runtimeRead.GameMode),
            runtimeRead.GameThreadTick,
            runtimeRead.Attempted,
            runtimeRead.Succeeded,
            !runtimeRead.Attempted
                ? "尚未读取游戏模式与 game tick。"
                : runtimeRead.Succeeded
                ? $"{FormatGameMode(runtimeRead.GameMode)}；tick={runtimeRead.GameThreadTick?.ToString() ?? "?"}。"
        ...
        Dictionary<string, DiagnosticStageState> stageOverrides;
        lock (_stageOverrideSync)
        {
            stageOverrides = new Dictionary<string, DiagnosticStageState>(
                _eventStageOverrides,
                StringComparer.Ordinal);
        }
        if (stageOverrides.Count > 0)
        {
            stages = stages
                .Select(stage => ApplyEventStageOverride(stage, stageOverrides))
                .ToArray();
        }
        ...
        var summary = health switch
        {
            ...
            TrainerDiagnosticHealth.Error => FormatErrorSummary(),
    ...
    private static DiagnosticStageSnapshot ApplyEventStageOverride(
        DiagnosticStageSnapshot stage,
        IReadOnlyDictionary<string, DiagnosticStageState> stageOverrides)
    {
        if (!stageOverrides.TryGetValue(stage.Id, out var escalated) ||
            StageRank(escalated) <= StageRank(stage.State))
        {
            return stage;
        }

        return stage with { State = escalated };
    }

    /// <summary>
    /// Health banner summary for the Error state (design doc §6.2): the classification code
    /// plus correlation id make the banner entry greppable against the event stream and the
    /// export bundle.
    /// </summary>
    private string FormatErrorSummary()
    {
        if (_lastDiagnosticFailure is not { } failure)
        {
            return _lastDiagnosticError ?? "检测到阻止当前会话继续工作的错误。";
        }

        var operationSuffix = failure.OperationId is { } operationId ? $"（op={operationId}）" : string.Empty;
        return $"{failure.Code}：{failure.Message}{operationSuffix}";
    }
    ...
    private sealed record RuntimeReadState(
        int? GameMode,
        uint? GameThreadTick,
        bool Attempted,
        bool Succeeded,
        bool Failed)
    {
        public static RuntimeReadState Empty { get; } = new(null, null, false, false, false);
    }
}
