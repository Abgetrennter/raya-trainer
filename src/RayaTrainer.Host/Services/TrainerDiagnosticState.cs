...
internal sealed class TrainerDiagnosticState
{
        ...
        int maxEvents = TrainerDiagnosticEventBuffer.Capacity)
    {
        ...
        if (_eventStageOverrides.Count > 0)
        {
            stages = stages
                .Select(ApplyEventStageOverride)
                .ToArray();
        }
    ...
    private DiagnosticStageSnapshot ApplyEventStageOverride(DiagnosticStageSnapshot stage)
    {
        if (!_eventStageOverrides.TryGetValue(stage.Id, out var escalated) ||
