...
/// <summary>
/// App 会话内的增援预设投影协调器（计划 R3）。只做三件事：
///   1. 接收 WPF 保存后的完整 presets 快照（保存成功后直接通知，不轮询设置文件）；
///   2. 维护 App 内 projectionSessionId / generation / 最近选择名会话缓存；
///   3. 在 Agent 就绪或快照变化时通过命令 62 原子替换 Agent 投影。
///
/// 会话身份：sessionId 是 App 启动时生成的非零 63 位随机值，App 重启即更换；即使接管
/// 仍在进程内的旧 Agent，也用新 session 原子替换投影并清空旧选择。generation 在同一
/// session 内严格递增。防竞态：同一时刻至多一个在途替换，发送期间再次保存只保证最后
/// 一个 generation 胜出；旧响应不会覆盖新状态。构建失败（超限等）时改发 Invalid 投影，
/// 让 Agent 禁用执行并保留用户可读错误。
/// </summary>
public sealed class ReinforcementPresetProjectionCoordinator
{
    private static readonly TimeSpan SyncTimeout = TimeSpan.FromSeconds(3);

    private readonly object _gate = new();
    private readonly IReinforcementPresetConsoleClient _client;
    private readonly Action<string?> _reportSyncStatus;
    private readonly ulong _sessionId;

    private ulong _generation;
    private IReadOnlyList<ReinforcementPreset>? _latestPresets;
    private IReadOnlyList<ReinforcementPreset>? _lastSyncedPresets;
    ...
    public ReinforcementPresetProjectionCoordinator(
        IReinforcementPresetConsoleClient? client = null,
        ...
        ulong? sessionId = null)
    {
        ...
        _sessionId = sessionId ?? ReinforcementPresetProjection.CreateSessionId();
    ...
    /// <summary>WPF 保存成功后推入完整快照。快照未变化时不重复同步。</summary>
    public void UpdatePresets(IReadOnlyList<ReinforcementPreset> presets)
    {
    ...
    /// <summary>
    /// 以低频（现有会话刷新节奏）从命令 63 读取 Agent 最近选择名，更新会话缓存。
    ...
    public async Task RefreshCachedSelectionAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var state = await _client
                .GetReinforcementPresetConsoleStateAsync(processId, SyncTimeout, cancellationToken)
                .ConfigureAwait(false);
            lock (_gate)
            {
                if (_agentReady &&
                    _processId == processId &&
                    state.AgentStatusCode == ReinforcementPresetConsoleWireCodec.AgentStatusOk &&
                    state.HasProjection &&
    ...
    private async Task SyncLoopAsync()
    {
        while (true)
        {
            ...
            ReplaceReinforcementProjectionResponse? response = null;
            var build = ReinforcementPresetProjectionBuilder.Build(presets, _sessionId, generation, preferred);
            ...
            if (!build.Success)
            {
                ...
                projection = ReinforcementPresetProjectionBuilder.BuildInvalid(
            ...
            try
            {
                response = await _client
                    .ReplaceReinforcementPresetProjectionAsync(processId, projection!, SyncTimeout)
                    .ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                statusError ??= $"增援预设同步失败：{ex.Message}";
            }
            ...
            lock (_gate)
            {
                ...
                if (isLatest && response is not null &&
                    response.AgentStatusCode == ReinforcementPresetConsoleWireCodec.AgentStatusOk)
                {
                    if (response.Accepted)
                    {
                        ...
                        if (build.Success)
                        {
                            _lastSyncedPresets = presets;
                             ...
                             response.RejectReason != ReinforcementProjectionRejectReason.GenerationRegressed)
                    {
                        statusError = $"Agent 拒绝了增援预设投影（{response.RejectReason}）。";
                    }
                }
                else if (isLatest && response is not null && statusError is null)
                {
                    statusError = $"增援预设同步失败：Agent 状态码 {response.AgentStatusCode}。";
                }
    ...
    private static bool PresetsEqual(
        IReadOnlyList<ReinforcementPreset> left,
        IReadOnlyList<ReinforcementPreset> right)
    {
