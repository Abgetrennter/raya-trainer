...
public enum ProductErrorCode : ushort
{
    ...
    // 4 retired with persistent offline admission in schema v3.
    QueueFull = 5,
    ResultExpired = 6,
    ProductUnavailable = 7,
    SchemaMismatch = 8,
    // Phase 1.1: CapturedTargetInvalid(9)+DependencyConflict(12) merged into IntentConflict.
    IntentConflict = 9,
    // Deprecated alias kept for wire-name parity with the native header; new code uses IntentConflict.
    CapturedTargetInvalid = 9,
    TargetRevisionMismatch = 11,
    // Deprecated: removed from the admission flow (native keeps the value for wire parity).
    PolicyStale = 13,
    ExecutionFault = 14,
