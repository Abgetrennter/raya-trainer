...
public static class ProductCatalogProjection
{
    ...
    /// product features and must never appear in WPF/Web catalogs. NativeWorkflow products
    /// (reinforcement.preset.execute) are Overlay-internal: their intent is built by the Agent
    /// from its own projection state, so WPF/Web offer no generic submit entry either.
    /// </summary>
    public static IReadOnlyList<ProductCatalogEntry> PublicEntries { get; } =
        Entries
            .Where(entry =>
                ...
                !IsNativeWorkflow(entry.ProductId.Value))
            .ToArray();
    ...
    private static bool IsNativeWorkflow(string productId) =>
        GeneratedProductDefinitionCatalog.Products.Any(definition =>
            string.Equals(definition.ProductId, productId, StringComparison.Ordinal) &&
            definition.Execution == GeneratedProductExecutionModel.NativeWorkflow);
    ...
    private static ReapplyPolicy MapReapply(GeneratedProductReapply reapply) => reapply switch
    {
        ...
        GeneratedProductReapply.OnReadyOnce => ReapplyPolicy.OnReadyOnce,
