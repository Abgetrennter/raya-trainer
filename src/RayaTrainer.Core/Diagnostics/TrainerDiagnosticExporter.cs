...
public static class TrainerDiagnosticExporter
{
        ...
        string? crashLogPath = null)
    {
        ...
        try
        {
            ...
            // Crash log reference (design doc §6.3): the last-resort log joins the bundle so
            // a FAULT surfaced on the diagnostics page can be matched to its full stack trace.
            if (!string.IsNullOrWhiteSpace(crashLogPath) && File.Exists(crashLogPath))
            {
                var crashLog = File.ReadAllText(crashLogPath);
                WriteText(
                    archive,
                    $"crash-log/{Path.GetFileName(crashLogPath)}",
                    RedactUserProfile(crashLog, userProfilePath));
            }
        }
        catch
        {
