...
- Native Hook 稳定 ID：`Private/Agent/NativeHookCatalog.cs`；hook 业务实现与分工见 `src/RayaTrainer.Agent/AGENTS.md`
...
- catalog 条目数：Native 侧 `AgentProtocol.h::NativeCatalogEntry::EntryCount`；托管侧不再维护镜像条目数，catalog 合同由 `Agent/Generated/RuntimeCatalogMetadata.generated.cs::CatalogContractHash` 描述（条目集合变化经生成器重算）
...
`scripts/validate-active-doc-drift.ps1` 会扫描 `AGENTS.md`、`docs/project-status.md`、`docs/backend-support-matrix.md`、各子树 `AGENTS.md`、`CLAUDE.md` 等，从代码读取协议版本/API 数/fingerprint/capability mask/settings schema 来核对文档。改这些文档时的隐性规则：

- 以 `|`（表格行）或 `-`（列表项）开头、且含 `YYYY-MM-DD` 日期的行被豁免，视为历史 changelog，不受漂移检查约束。
- 因此**保留旧协议号/旧 fingerprint 的历史叙述时，必须让该行符合 `|`/`-` 开头 + 日期格式**；否则会触发漂移违规。
- 正文中直接写当前协议/fingerprint/capability 数值时，必须与代码一致（当前从 `AgentProtocol.cs`/`apis.json`/`RuntimeCatalogMetadata.generated.cs` 读取）。
