...
public static class AscensionModifierCatalog
{
    ...
    public const int RangeAttributeType = 7;
    public const int VisionAttributeType = 17;

    /// <summary>
    /// 属性修改矩阵的一行：一到两个 attributeType 共享同一开关/倍率/作用范围。
    /// RANGE 与 VISION 通常同时出现，合并为一行（应用时同一量化值下发两个类型）；
    /// 合并行占用 RANGE 在 XSD 枚举序中的位置。
    /// </summary>
    public sealed record AscensionMatrixRow(IReadOnlyList<AscensionModifierEntry> Entries, string? Label = null)
    {
        public string DisplayName => Label ?? Entries[0].DisplayName;
    }

    public static IReadOnlyList<AscensionMatrixRow> MatrixRows { get; } = BuildMatrixRows();

    private static IReadOnlyList<AscensionMatrixRow> BuildMatrixRows()
    {
        var range = All.Single(e => e.AttributeType == RangeAttributeType);
        var vision = All.Single(e => e.AttributeType == VisionAttributeType);
        var merged = new AscensionMatrixRow([range, vision], "攻击射程 + 索敌");
        // 合并条目必须共用同一量化阶梯（两者均为 Additive）。
        if (range.Aggregation != vision.Aggregation)
        {
            throw new InvalidOperationException("RANGE 与 VISION 的聚合分类不一致，无法合并矩阵行。");
        }

        return All
            .Where(e => e.AttributeType != VisionAttributeType)
            .Select(e => e.AttributeType == RangeAttributeType ? merged : new AscensionMatrixRow([e]))
            .ToArray();
    }
