...
/// 顺序对应 MainWindow.xaml 侧边栏索引（0-8，公共外壳共 9 页）。
/// operation-explorer 只存在于私有构建（索引 9），由 App 私有分部承接其索引/持久化映射，不进公共 InIndexOrder。
/// </summary>
public static class PageIds
{
    ...
    public const string ProductConsole = "product-console"; // 8

    public static readonly IReadOnlyList<string> InIndexOrder =
    [
        Features, SelectedUnit, Reinforcement, SecretProtocol,
        Tools, StatusEditor, Diagnostics, HotkeySettings, ProductConsole
    ];
