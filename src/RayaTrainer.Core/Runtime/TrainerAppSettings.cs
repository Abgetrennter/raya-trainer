...
public sealed record TrainerAppSettings
{
    ...
    public TrainerAppSettings(
        ...
        DurableProductPolicy? DurableProductPolicy = null,
        bool HideDeveloperSurfaces =
#if DEBUG
            false
#else
            true
...
#endif
        ,
        IReadOnlyList<AscensionPreset>? AscensionPresets = null
        )
    {
        ...
        this.DurableProductPolicy = DurableProductPolicy ?? Features.DurableProductPolicy.Empty;
        this.HideDeveloperSurfaces = HideDeveloperSurfaces;
        this.EnableWebControl = EnableWebControl;
        this.AscensionPresets = (AscensionPresets ?? Array.Empty<AscensionPreset>()).ToArray();
    }
    ...
    /// <summary>
    /// Durable product policy imported by Core (contract §10). Persists only ProductId,
    /// parameters, symbolic (Rebindable) scope, whether Reapply is enabled and a monotonic
    /// Policy Revision. Captured Target / token are never stored here.
    /// </summary>
    public DurableProductPolicy DurableProductPolicy { get; init; }

    /// <summary>
    /// 是否隐藏开发者/高级控制面（产品控制台、脚本操作（高级）等未完工入口）。
    /// 默认值由编译配置决定：Debug 默认显示（false），Release 默认隐藏（true）。
    /// </summary>
    public bool HideDeveloperSurfaces { get; init; }

    /// <summary>
    /// 是否启用网页/手机遥控入口（局域网 HTTP 服务，端口 8787）。
    /// 默认值由编译配置决定：Debug 默认开启（true，便于联调），Release 默认关闭（false），
    /// 以确保发布版程序启动时不会监听任何网络端口（默认完全离线）。
    /// 用户可在“工具 → 手机遥控”手动开启；开启并重启程序后才会创建本地 HTTP 服务。
    /// 该服务不建立任何出站网络连接，仅供局域网内手机浏览器访问。
    /// </summary>
    public bool EnableWebControl { get; init; }

    public IReadOnlyList<AscensionPreset> AscensionPresets { get; init; }
}
...
public sealed class TrainerAppSettingsStore
{
    ...
    private static TrainerAppSettings Normalize(JsonElement root, TrainerAppSettings defaults)
    {
        ...
        var ascensionPresets = ReadAscensionPresets(root);
        var lastAppliedPreset = ReadString(root, nameof(TrainerAppSettings.LastAppliedFeaturePresetName));
        var durableProductPolicy = ReadDurableProductPolicy(root);
        var hideDeveloperSurfaces = ReadBool(root, nameof(TrainerAppSettings.HideDeveloperSurfaces))
            ?? defaults.HideDeveloperSurfaces;
        var enableWebControl = ReadBool(root, nameof(TrainerAppSettings.EnableWebControl))
            ?? defaults.EnableWebControl;
        ...
        return new TrainerAppSettings(
            ...
            featurePresets,
            lastAppliedPreset,
            durableProductPolicy,
            hideDeveloperSurfaces,
            enableWebControl,
            ascensionPresets);
    }

    private static DurableProductPolicy ReadDurableProductPolicy(JsonElement root)
    {
        if (!root.TryGetProperty(nameof(TrainerAppSettings.DurableProductPolicy), out var value) ||
            value.ValueKind != JsonValueKind.Object)
        {
            return DurableProductPolicy.Empty;
        }

        // Invalid entries (bad ProductId, captured/unknown scope, too many parameters or
        // >64 entries) are isolated with diagnostics; the rest of the settings are preserved.
        return DurableProductPolicy.FromJsonElement(value, out _);
    }
    ...
    private static IReadOnlyList<AscensionPreset> ReadAscensionPresets(JsonElement root)
    {
        if (!root.TryGetProperty(nameof(TrainerAppSettings.AscensionPresets), out var value) ||
            value.ValueKind != JsonValueKind.Array)
        {
            return Array.Empty<AscensionPreset>();
        }

        var presets = new List<AscensionPreset>();
        foreach (var item in value.EnumerateArray())
        {
            if (item.ValueKind != JsonValueKind.Object) continue;

            var name = ReadString(item, nameof(AscensionPreset.Name));
            if (string.IsNullOrWhiteSpace(name)) continue;

            var faction = ReadInt32(item, nameof(AscensionPreset.Faction)) ?? 0;
            if (faction is < 0 or > 2) faction = 0;

            var rows = new List<AscensionPresetRow>();
            if (item.TryGetProperty(nameof(AscensionPreset.Rows), out var rowsElement) &&
                rowsElement.ValueKind == JsonValueKind.Array)
            {
                foreach (var row in rowsElement.EnumerateArray())
                {
                    if (row.ValueKind != JsonValueKind.Object) continue;
                    var attributeType = ReadInt32(row, nameof(AscensionPresetRow.AttributeType));
                    if (attributeType is null) continue;

                    rows.Add(new AscensionPresetRow(
                        attributeType.Value,
                        ReadBool(row, nameof(AscensionPresetRow.IsEnabled)) ?? false,
                        ReadBool(row, nameof(AscensionPresetRow.IsUp)) ?? true,
                        ReadString(row, nameof(AscensionPresetRow.MultiplierText)) ?? "2",
                        ReadBool(row, nameof(AscensionPresetRow.ScopeSelected)) ?? true,
                        ReadBool(row, nameof(AscensionPresetRow.ScopeAll)) ?? false,
                        ReadBool(row, nameof(AscensionPresetRow.ScopeNewRegistered)) ?? false));
                }
            }

            var createdAt = ReadDateTimeOffset(item, nameof(AscensionPreset.CreatedAtUtc)) ?? DateTimeOffset.UtcNow;
            var updatedAt = ReadDateTimeOffset(item, nameof(AscensionPreset.UpdatedAtUtc)) ?? createdAt;
            presets.Add(new AscensionPreset(name.Trim(), faction, rows, createdAt, updatedAt));
        }
        return presets;
    }
