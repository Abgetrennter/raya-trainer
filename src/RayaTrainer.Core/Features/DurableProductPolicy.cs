...
/// <see cref="ScopeKind.FixedPlayer"/>). Captured Target / token therefore never
/// reach serialization.
///
...
public sealed class DurableProductPolicy
{
    ...
    public static DurablePolicyDiagnostic? Validate(DurablePolicyEntry entry)
    {
        ...
        if (entry.ReapplyPolicy is not (ReapplyPolicy.None or ReapplyPolicy.OnMapStart))
        {
            return new DurablePolicyDiagnostic(
                DurablePolicyDiagnosticCode.InvalidReapplyPolicy,
                $"ReapplyPolicy '{entry.ReapplyPolicy}' is not allowed; only None/OnMapStart.",
                entry.PolicyEntryId,
                entry.ProductId.Value);
        }
    ...
    private static DurablePolicyEntry? ParseEntry(JsonElement item, List<DurablePolicyDiagnostic> diags)
    {
        ...
        if (string.Equals(reapplyText, "OnReadyOnce", StringComparison.OrdinalIgnoreCase))
        {
            // One-time migration from the pre-rename settings shape.
            reapply = ReapplyPolicy.OnMapStart;
        }
        else if (!Enum.TryParse<ReapplyPolicy>(reapplyText, ignoreCase: true, out reapply))
        {
            diags.Add(new DurablePolicyDiagnostic(
                DurablePolicyDiagnosticCode.InvalidReapplyPolicy,
