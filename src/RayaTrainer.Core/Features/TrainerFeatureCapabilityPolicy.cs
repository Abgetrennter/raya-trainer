...
public sealed class TrainerFeatureCapabilityPolicy
{
    ...
    /// <summary>
    /// Agent-owned runtime (plan §498): the Agent self-resolves the upgrade-grant engine
    /// layout, so the App no longer reads profile Native refs. The gate is purely the
    /// supported unit-upgrade profile id.
    /// </summary>
    public static bool IsUnitUpgradeNativeLayoutReady(Ra3VersionProfile profile)
    {
        return string.Equals(profile.Id, "ra3_1.12", System.StringComparison.OrdinalIgnoreCase);
    }
}
