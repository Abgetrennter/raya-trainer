...
public sealed record FeaturePreset(
    ...
    DateTimeOffset UpdatedAtUtc) : INamedPreset;

/// <summary>
/// 属性修改矩阵中的单行配置。AttributeType 是稳定的 Native 类型编号；
/// ActiveNote 属于运行时读回状态，不进入预设。
/// </summary>
public sealed record AscensionPresetRow(
    int AttributeType,
    bool IsEnabled,
    bool IsUp,
    ...
    bool ScopeSelected,
    bool ScopeAll,
    bool ScopeNewRegistered);

/// <summary>
/// 属性修改矩阵的命名预设。Faction 使用与 Agent 合同一致的 0=我方、1=敌方、2=全部。
/// </summary>
public sealed record AscensionPreset(
    string Name,
    int Faction,
    IReadOnlyList<AscensionPresetRow> Rows,
    DateTimeOffset CreatedAtUtc,
    DateTimeOffset UpdatedAtUtc) : INamedPreset;
