...
public sealed partial class AgentFeatureController : IAgentFeatureController
{
    ...
    {
        _client = client;
        InitializePrivateFeatureAdapter(client);
    ...
    {
        var handledByPrivateAdapter = false;
        var privateStatus = GameApiDispatchStatus.Failed;
        TryExecutePrivateProductToggle(feature, enabled, ref handledByPrivateAdapter, ref privateStatus);
        if (handledByPrivateAdapter)
        {
            if (privateStatus != GameApiDispatchStatus.Completed)
            {
                throw new InvalidOperationException($"{feature.DisplayName} 的私有产品执行未完成：{privateStatus}。");
            }
            return;
        }
    ...
    {
        var handledByPrivateAdapter = false;
        TryResetPrivateProduct(feature, ref handledByPrivateAdapter);
        if (handledByPrivateAdapter)
        {
            return;
        }
        ...
        TimeSpan timeout)
    {
        var handledByPrivateAdapter = false;
        var privateStatus = GameApiDispatchStatus.Failed;
        TryExecutePrivateProductAction(feature, ref handledByPrivateAdapter, ref privateStatus);
        if (handledByPrivateAdapter)
        {
        ...
        TimeSpan timeout)
    {
        switch (actionId)
        {
            ...
            // cmd 42-45（攻速/射程切换与清除）已随 flag 路径退役（迁移计划 Phase 5）；
    ...
    public bool? ReadToggleState(TrainerFeature feature)
    {
        ...
        var handledByPrivateAdapter = false;
        bool? privateState = null;
        TryReadPrivateProductToggle(feature, ref handledByPrivateAdapter, ref privateState);
        if (handledByPrivateAdapter)
        {
            return privateState;
        }
    ...
    partial void InitializePrivateFeatureAdapter(IAgentClient client);

    partial void TryExecutePrivateProductToggle(
        TrainerFeature feature,
        bool enabled,
        ref bool handled,
        ref GameApiDispatchStatus status);

    partial void TryExecutePrivateProductAction(
        TrainerFeature feature,
        ref bool handled,
        ref GameApiDispatchStatus status);

    partial void TryResetPrivateProduct(TrainerFeature feature, ref bool handled);

    partial void TryReadPrivateProductToggle(
        TrainerFeature feature,
        ref bool handled,
        ref bool? state);
