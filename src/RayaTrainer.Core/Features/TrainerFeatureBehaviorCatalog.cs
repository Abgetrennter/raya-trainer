...
public static class TrainerFeatureBehaviorCatalog
{
    ...
    /// the Agent reports either its legacy full transaction or the readiness of its layered optional
    /// transaction, so the App never needs to scan signatures or consume profile Native refs.
    /// </summary>
    ...
    private static FrozenDictionary<string, TrainerFeatureBehavior> BuildCatalog()
    {
        var entries = new List<TrainerFeatureBehavior>(72)
        {
            ...
            ProductIntent(TrainerFeatureIds.ProductSpawnArsenalMcv,    "spawn.arsenal-mcv"),
