...
public sealed class TargetProcessHeartbeatMonitor : IDisposable
{
    ...
    // Safe one-shot liveness check for callers outside the polling loop (e.g. a user-triggered
    // manual detect): returns false when the process is gone instead of throwing. The periodic
    // loop keeps using IsProcessAlive directly under its own try/catch.
