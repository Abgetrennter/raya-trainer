...
internal sealed class ProductControlSession : IProductControlSession
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Identity gate (plan §7.3): only a wire-incompatible protocol major blocks the plane.
        // A different BuildId at the same protocol major no longer refuses the plane — per-feature
        // gaps are handled by the capability/schema gates below.
        if (status.AgentVersion != AgentProtocol.Version)
        {
            return Store(baseline with
            {
                Status = ProductControlStatus.FingerprintMismatch,
                Detail =
                    $"Agent protocol {status.AgentVersion} != required {AgentProtocol.Version}.",
            });
        }

        // Capability gate: the plane is optional. A target that never advertised MatchContext
        // + ProductControlPlane costs no pipe round-trip.
        if (!matchContext || !productPlane)
        {
            return Store(baseline with
            {
                Status = ProductControlStatus.CapabilityUnavailable,
                Detail =
                    $"granted=0x{granted:X}; MatchContext={matchContext}; ProductControlPlane={productPlane}.",
            });
        }
...
internal static class ProductControlDiagnosticsBuilder
{
        ...
        ProductResult? latestResult = null)
    {
        ...
        var singlePlayerProven = contextOk && context!.SinglePlayerProof == SinglePlayerProof.Proven;
        ...
        return new ProductControlDiagnostics
        {
            ...
            StatusSummary = BuildStatusSummary(negotiation.Status, lifecycle, singlePlayerProven),
        };
    }
        ...
        bool singlePlayerProven)
    {
        switch (status)
        {
            ...
            case ProductControlStatus.FingerprintMismatch:
                return "Agent 版本与修改器不一致，产品控制不可用；请重启游戏并重新连接。";
        ...
        return lifecycle switch
        {
            ...
            MatchLifecycle.Ready when singlePlayerProven =>
                "对局已就绪，可以下发产品指令。",
            MatchLifecycle.Ready =>
                "对局已就绪，但尚未确认为单人对局，部分产品指令会被拒绝。",
            MatchLifecycle.Loading or MatchLifecycle.Resolving =>
                "对局正在加载，产品指令会在对局就绪后自动可用，请稍候。",
            MatchLifecycle.Invalidated =>
                "上一局已结束，产品指令将在下一局开始后可用。",
