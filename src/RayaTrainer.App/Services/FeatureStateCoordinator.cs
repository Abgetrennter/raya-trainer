...
/// </summary>
public sealed class FeatureStateCoordinator : IFeatureToggleCoordinator
{
    ...
    /// <summary>
    /// Web 遥控开关入口（IFeatureToggleCoordinator）：ViewModel 非线程安全，
    /// 在非 UI 线程调用时经 WPF Dispatcher 调度回 UI 线程。
    /// </summary>
    public Task<bool> TrySetToggleDesiredAsync(string rawName, bool enabled)
    {
        var item = FindItem(rawName);
        if (item is null)
        {
        ...
        if (dispatcher is not null && !dispatcher.CheckAccess())
        {
            ...
            dispatcher.InvokeAsync(() =>
            {
                try
                {
                    item.SetDesired(enabled, suppressApply: false);
