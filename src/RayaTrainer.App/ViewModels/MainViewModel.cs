...
public sealed partial class MainViewModel : ViewModelBase, IFeatureHost, IDisposable
{
    ...
    async Task<(bool Success, string Message)> IFeatureHost.ExecuteProductIntentFeatureAsync(TrainerFeature feature)
    {
        ...
        // 生成矿脉带 angle 参数（整数度），预校验与 Native 侧 InvalidRequest 规则一致
        // （GameObjectSpawnProductIntent.cpp：[-360, 360]）。
        if (feature.RawName == TrainerFeatureIds.ProductSpawnOreNode)
        {
            var text = FeatureToggle.OreNodeAngleText.Trim();
            if (!int.TryParse(
                    text,
                    System.Globalization.NumberStyles.Integer,
                    System.Globalization.CultureInfo.InvariantCulture,
                ...
                angle < -360 || angle > 360)
            {
                return (false, "请先在生成矿脉的朝向输入框填写 -360~360 的整数角度（0=朝东，常用 45/135/-45/-135）。");
            }

            parameters = [ScriptValue.Integer(angle)];
        }
