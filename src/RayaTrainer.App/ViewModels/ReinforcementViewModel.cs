...
public sealed class ReinforcementViewModel : ViewModelBase
{
        ...
        TrainerAppSettings settings)
    {
        ...
        DeleteReinforcementPresetCommand = new RelayCommand(DeleteSelectedReinforcementPreset, () => SelectedReinforcementPreset is not null && !_isQueueRunning());
    ...
    public string DeleteReinforcementPresetHelpText => "删除选中的预设；游戏内控制台会自动刷新并清空对应选择。";
    ...
    public RelayCommand DeleteReinforcementPresetCommand { get; }
    ...
    private void SaveReinforcementPreset()
    {
        try
        {
            ...
            NamedPresetList.Upsert(ReinforcementPresets, preset);
    ...
    private void DeleteSelectedReinforcementPreset()
    {
        if (SelectedReinforcementPreset is null) return;
        var name = SelectedReinforcementPreset.Name;
        if (!NamedPresetList.RemoveByName(ReinforcementPresets, name)) return;
        SelectedReinforcementPreset = null;
        // 持久化 + 投影同步：Overlay 的同名选择会被 Agent 清空并禁用执行。
        _persistSettings();
        _setStatus($"已删除增援队列预设：{name}");
    }
    ...
    public void RaiseCommandStates()
    {
        ...
        DeleteReinforcementPresetCommand.RaiseCanExecuteChanged();
