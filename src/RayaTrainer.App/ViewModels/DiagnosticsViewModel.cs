...
public sealed class DiagnosticsViewModel : ViewModelBase, IDisposable
{
    ...
    public string OverlayDetails => Snapshot.Overlay.Summary;

    // --- 产品控制（Product Control Plane，I4/U3）---
    // 全部派生自快照上的只读 ProductControlDiagnostics，只暴露分层公开状态与一句中文
    // 说明，绝不暴露内部指针或 Route 字符串（U3 完成标准）。

    private ProductControlDiagnostics? ProductControl => Snapshot.ProductControl;

    public bool HasProductControl => ProductControl is not null;

    // 面向新手的一句话“下一步”，回答“为什么按钮不可用/为什么没生效/是否等待下一局”。
    public string ProductControlStatusSummary =>
        ProductControl?.StatusSummary is { Length: > 0 } summary
            ? summary
            : "尚未采集产品控制状态。";

    public string ProductControlCapabilityDetails
    {
        get
        {
            if (ProductControl is not { } pc)
            {
                return "-";
            }

            var negotiated = pc.CapabilityNegotiated ? "已协商" : "未协商";
            var match = pc.MatchContextCapable ? "已授权" : "未授权";
            var plane = pc.ProductControlPlaneCapable ? "已授权" : "未授权";
            return $"能力协商：{negotiated} · 对局上下文：{match} · 产品控制面：{plane}（0x{pc.GrantedCapabilities:X}）";
        }
    }

    public string ProductControlSchemaDetails =>
        ProductControl is { } pc
            ? $"上下文 Schema v{pc.ContextSchemaVersion} · 产品控制 Schema v{pc.ProductControlSchemaVersion}"
            : "-";

    public string ProductControlLifecycleDetails
    {
        get
        {
            if (ProductControl is not { } pc)
            ...
            var activePlayers = pc.ActivePlayerCount == 0
                ? "未知"
                : pc.ActivePlayerCount.ToString(System.Globalization.CultureInfo.InvariantCulture);
            return $"对局阶段：{TranslateLifecycle(pc.MatchLifecycle)} · 在场玩家：{activePlayers}";
        }
    }

    public string ProductControlSnapshotDetails =>
        ProductControl is { } pc
            ? $"快照代次：{pc.SnapshotGeneration}"
            : "-";

    public string ProductControlDesiredDetails =>
        ProductControl is { } pc
            ? $"目标意图：共 {pc.DesiredTotalCount} · 待生效 {pc.DesiredPendingCount} · 生效中 {pc.DesiredActiveCount}" +
              $" · 已停用 {pc.DesiredDisabledCount} · 被取代 {pc.DesiredSupersededCount} · 策略版本 {pc.PolicyRevision}"
            : "-";

    public string ProductControlLatestResultDetails
    {
        get
        {
            if (ProductControl is not { } pc)
            ...
            if (pc.LastSubmittedIntentId == 0 && string.IsNullOrEmpty(pc.LastResultProductId))
            {
                return "尚未下发产品指令。";
            }

            var product = string.IsNullOrEmpty(pc.LastResultProductId) ? "-" : pc.LastResultProductId;
            var line = $"最近产品：{product}（意图 #{pc.LastSubmittedIntentId}）" +
                $" · 准入：{Dash(pc.LastAdmissionState)} · 执行：{Dash(pc.LastExecutionState)}" +
                $" · 效果：{Dash(pc.LastEffectState)} · 补偿：{Dash(pc.LastCompensationState)}";
            return string.IsNullOrEmpty(pc.LastErrorCode)
                ? line
                : $"{line} · 错误：{pc.LastErrorCode}";
        }
    }

    private static string Dash(string value) => string.IsNullOrEmpty(value) ? "-" : value;

    private static string TranslateLifecycle(string lifecycle) => lifecycle switch
    {
        "Ready" => "就绪",
        "Loading" => "加载中",
        "Resolving" => "解析中",
    ...
    public string TechnicalDetails
    {
        get
        {
            ...
            var lines = new List<string>
            {
                ...
                $"overlay.applicable={overlay.Applicable}",
                $"overlay.status={overlay.StatusCode?.ToString() ?? "-"}",
                $"overlay.lifecycle={overlay.Lifecycle}",
                $"overlay.flags=0x{(uint)overlay.Flags:X8}",
                $"overlay.frames={overlay.RenderFrameCount}",
                $"overlay.clicks={overlay.ButtonClickCount}",
                $"overlay.resets={overlay.DeviceResetCount}",
                $"overlay.error={overlay.LastError}"
            };
    ...
    private void CheckLaa()
    {
        ...
        _source?.RecordDiagnosticEvent(
            ...
            _laaResult is null
                ? DiagnosticEventCodes.LaaCheckFailed
                : _laaResult.Value
                    ? DiagnosticEventCodes.LaaFound
                    : DiagnosticEventCodes.LaaMissing,
            _laaResult switch
            {
                null => "LAA 标记检查失败。",
                true => "LAA 标记已存在。",
    ...
    private void PatchLaa()
    {
        ...
        if (result.Success)
        {
            ...
            _source?.RecordDiagnosticEvent(
                DiagnosticEventSeverity.Info,
                DiagnosticEventCodes.LaaPatched,
                "LAA 标记已成功写入。",
                _laaModulePath);
        }
        else
        {
            ...
            _source?.RecordDiagnosticEvent(
                DiagnosticEventSeverity.Error,
                DiagnosticEventCodes.LaaPatchFailed,
                "LAA 标记写入失败。",
                result.ErrorMessage);
        }
    ...
    private void ExportDiagnostics()
    {
        ...
        try
        {
            TrainerDiagnosticExporter.Export(
                ...
                crashLogPath: Path.Combine(AppContext.BaseDirectory, "RayaTrainer_crash.log"));
            _source?.RecordDiagnosticEvent(
                DiagnosticEventSeverity.Info,
                DiagnosticEventCodes.DiagnosticsExported,
                "诊断包已导出。",
                dialog.FileName);
        ...
        catch (Exception ex)
        {
            _source?.RecordDiagnosticEvent(
                DiagnosticEventSeverity.Error,
                DiagnosticEventCodes.DiagnosticsExportFailed,
                "诊断包导出失败。",
                ex.Message);
    ...
    private void RaiseSummaryProperties()
    {
        ...
        OnPropertyChanged(nameof(OverlayDetails));
        OnPropertyChanged(nameof(HasProductControl));
        OnPropertyChanged(nameof(ProductControlStatusSummary));
        OnPropertyChanged(nameof(ProductControlCapabilityDetails));
        OnPropertyChanged(nameof(ProductControlSchemaDetails));
        OnPropertyChanged(nameof(ProductControlLifecycleDetails));
        OnPropertyChanged(nameof(ProductControlSnapshotDetails));
        OnPropertyChanged(nameof(ProductControlDesiredDetails));
        OnPropertyChanged(nameof(ProductControlLatestResultDetails));
