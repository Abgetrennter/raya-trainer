...
public sealed class DiagnosticsViewModel : ViewModelBase, IDisposable
{
    ...
    public string ProductControlLifecycleDetails
    {
        get
        {
            ...
            var single = pc.SinglePlayerProven ? "已确认单人对局" : "未确认单人对局";
            var activePlayers = pc.ActivePlayerCount == 0
                ? "未知"
                : pc.ActivePlayerCount.ToString(System.Globalization.CultureInfo.InvariantCulture);
            return $"对局阶段：{TranslateLifecycle(pc.MatchLifecycle)} · 在场玩家：{activePlayers} · {single}";
        }
    }

    public string ProductControlEpochDetails =>
        ProductControl is { } pc
            ? $"地图纪元：{pc.MapEpoch} · 快照代次：{pc.SnapshotGeneration}"
            : "-";
    ...
    private void RaiseSummaryProperties()
    {
        ...
        OnPropertyChanged(nameof(ProductControlEpochDetails));
