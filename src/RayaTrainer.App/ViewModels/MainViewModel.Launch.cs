...
public sealed partial class MainViewModel
{
    ...
    public void DetectProcessOnDemand()
    {
        if (TryKeepCurrentTarget())
        {
            _ = Diagnostics.RefreshNowAsync();
        }
        else
        {
            RefreshProcess();
    ...
    private async Task DetectProcessOnDemandAsync()
    {
        if (TryKeepCurrentTarget())
        {
    ...
    private void AttachTarget(TrainerTarget target, bool autoInstall, string? notice = null)
    {
        try
        {
            ...
            ApplyAttachResult(target, autoInstall, notice, result);
        }
        catch (Exception ex)
        {
    ...
    private void ApplyAttachResult(
