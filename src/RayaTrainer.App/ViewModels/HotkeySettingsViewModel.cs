...
public sealed class HotkeySettingsViewModel : ViewModelBase
{
    ...
    public const string ToggleOverlayRawName = TrainerFeatureIds.ToggleOverlay;
    ...
    public HotkeySettingsViewModel(
        ...
        IReadOnlyList<ActionHotkeyDefinition> actionHotkeys,
        ...
        Action<IReadOnlyDictionary<string, string>> applyChanges)
    {
        ...
        BuildRows(features, actionHotkeys, currentHotkeys, defaultHotkeys);
        RecomputeConflicts();
    }
    ...
    /// <summary>请求定位某行时触发（携带行 VM），由设置页滚动到可见并高亮。</summary>
    public event Action<HotkeyRowViewModel>? RevealRequested;

    /// <summary>跳转入口：按 RawName 定位行并请求滚动高亮；找不到时静默忽略。</summary>
    public void RequestReveal(string? rawName)
    {
        ...
        var row = Rows.FirstOrDefault(r => r.RawName == rawName);
        if (row is not null)
        {
        ...
        IReadOnlyDictionary<string, string> defaultHotkeys)
    {
        ...
        // 动作热键行直接来自 MainViewModel 声明表；全局注册的动作单独分组，便于用户识别。
        foreach (var definition in actionHotkeys)
        {
            AddActionRow(definition.RawName, definition.DisplayName, currentHotkeys, defaultHotkeys,
                definition.Scope == ActionHotkeyScope.GlobalRegister ? TrainerControlGroupName : ActionGroupName);
        }
...
public sealed class HotkeyRowViewModel : ViewModelBase
{
    ...
    /// <summary>跳转定位高亮：由设置页在滚动定位时置位、超时后清除，用于视觉引导。</summary>
