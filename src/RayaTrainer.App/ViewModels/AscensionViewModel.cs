...
public sealed class AscensionViewModel : ViewModelBase
{
    ...
    // 按钮文本随动作热键声明表刷新（未分配时显示纯标签），由 MainViewModel.ReloadHotkeys 驱动。
    private string _applyButtonText = "应用";
    public string ApplyButtonText
    {
        get => _applyButtonText;
        internal set
        {
            if (string.Equals(_applyButtonText, value, StringComparison.Ordinal))
            {
                return;
            }
    ...
    private string _restoreButtonText = "全部还原";
    public string RestoreButtonText
    {
        get => _restoreButtonText;
        internal set
        {
            if (string.Equals(_restoreButtonText, value, StringComparison.Ordinal))
            {
                return;
            }

            _restoreButtonText = value;
            OnPropertyChanged();
        }
    }
    ...
    internal async Task ApplyAsync()
    {
        ...
        var overridden = pendingPersistent.SelectMany(KeysOf).ToHashSet();
        var merged = _committed
            .Where(kv => !overridden.Contains(kv.Key))
            .ToDictionary(kv => kv.Key, kv => kv.Value);
        foreach (var row in pendingPersistent)
        {
            var entry = new CommittedEntry(row.Quantization.WeightValues.ToList(), ScopeMaskOf(row));
            foreach (var key in KeysOf(row))
            {
                merged[key] = entry;
            }
        }
    ...
    {
        var targets = rowsToClear.Where(r => KeysOf(r).Any(_committed.ContainsKey)).ToList();
        if (targets.Count == 0)
        {
            return;
        }

        var clearedKeys = targets.SelectMany(KeysOf).Distinct().ToHashSet();
    ...
    {
        var committedRows = side.AllRows().Where(r => KeysOf(r).Any(_committed.ContainsKey)).ToList();
    ...
    private void ApplyCommittedState(IReadOnlyDictionary<(AscensionSide Side, int Type), CommittedEntry> committed)
    {
        ...
        {
            row.ActiveNote = RowActiveText(row, "已生效：");
        }
        RebuildOverview();
    }

    // 合并行的生效文本按条目分别列出（读回后两个类型的值可能不一致）；普通行保持原样。
    private string RowActiveText(AscensionRowViewModel row, string prefix)
    {
        var parts = new List<string>();
        foreach (var entry in row.Entries)
        {
            if (_committed.TryGetValue((row.Side, entry.AttributeType), out var state) && state.TemplateValues.Count > 0)
            {
                var values = string.Join("，", state.TemplateValues.Select(FormatTemplateValue));
                parts.Add(row.Entries.Count == 1 ? values : $"{entry.DisplayName} {values}");
            }
        }
    ...
    private void RebuildOverview()
    {
        foreach (var side in Sides)
        {
            ...
            {
                if (RowActiveText(row, string.Empty) is not { Length: > 0 } valueText)
                {
                    continue;
                }

                // 合并行是一个修改：清除时两个类型一起清；作用范围取各条目提交掩码的拼合。
                var scopeMask = row.AttributeTypes.Aggregate(
                    0u, (mask, type) => mask | (_committed.TryGetValue((side.Side, type), out var state) ? state.ScopeMask : 0u));
                side.ActiveItems.Add(new AscensionActiveItemViewModel(
                    row.DisplayName,
                    valueText,
                    ScopeMaskSummary(scopeMask),
    ...
    // One wire row per (quantized template × attribute type): "attributeType:valueBits:scopeMask:faction";
    // each row carries the faction code of the side tab it was configured on. The scope mask mirrors the
    ...
    internal static IReadOnlyList<string> BuildWireRows(IReadOnlyList<AscensionRowViewModel> enabledRows)
    {
        ...
        foreach (var row in enabledRows)
        {
            ...
            foreach (var weightValue in row.Quantization.WeightValues)
            {
                ...
                foreach (var type in row.AttributeTypes)
                {
                    rows.Add($"{type}:{valueBits}:{scopeMask}:{(int)row.Side}");
                }
            }
        }
        return rows;
    ...
    private static IEnumerable<(AscensionSide Side, int Type)> KeysOf(AscensionRowViewModel row) =>
        row.AttributeTypes.Select(type => (row.Side, type));
    ...
    private void LoadSelectedPreset()
    {
        ...
        foreach (var row in side.AllRows())
        {
            // 旧预设把合并前的 RANGE/VISION 存为两行：合并行优先取主类型，缺省回落链接类型。
            var presetRow = savedRows.GetValueOrDefault(row.Entry.AttributeType)
                ?? row.Entries.Skip(1)
                    .Select(e => savedRows.GetValueOrDefault(e.AttributeType))
                    .FirstOrDefault(r => r is not null);
            row.RestorePreset(presetRow);
        }
    ...
    private AscensionPreset CapturePreset(string name, DateTimeOffset createdAt, DateTimeOffset updatedAt)
    {
        ...
        return new AscensionPreset(
            ...
            (int)side.Side,
            side.AllRows()
                .Select(r => r.CapturePreset(KeysOf(r).Any(_committed.ContainsKey)))
                .ToArray(),
            createdAt,
            updatedAt);
    }
...
public sealed class AscensionSideViewModel : ViewModelBase
{
    ...
    private static IReadOnlyList<AscensionGroupViewModel> BuildGroups(AscensionSide side)
    {
        ...
        {
            var rows = AscensionModifierCatalog.MatrixRows
                .Where(r => r.Entries[0].GroupId == groupId)
                .Select(r => new AscensionRowViewModel(r.Entries, side, r.Label))
                .ToList();
            if (rows.Count > 0)
            {
...
public sealed class AscensionRowViewModel : ViewModelBase
{
    ...
    public AscensionRowViewModel(IReadOnlyList<AscensionModifierEntry> entries, AscensionSide side, string? displayName = null)
    {
        Entries = entries;
        Side = side;
        _displayName = displayName;
        _quantization = AscensionQuantizer.Quantize(entries[0].Aggregation, _multiplierText, invert: !_isUp);
    }

    /// <summary>主条目（驱动量化阶梯与技术名显示；合并行的其余条目见 <see cref="Entries"/>）。</summary>
    public AscensionModifierEntry Entry => Entries[0];

    /// <summary>本行覆盖的条目（一到两个）；合并行应用时同一量化值下发全部条目类型。</summary>
    public IReadOnlyList<AscensionModifierEntry> Entries { get; }

    public IEnumerable<int> AttributeTypes => Entries.Select(e => e.AttributeType);

    private string? _displayName;

    /// <summary>行显示名（合并行用矩阵标签，普通行即主条目显示名）。</summary>
    public string DisplayName => _displayName ?? (Entries.Count == 1
        ? Entries[0].DisplayName
        : string.Join(" + ", Entries.Select(e => e.DisplayName)));

    /// <summary>技术名（合并行为各条目枚举名连接）。</summary>
    public string EnumName => string.Join("+", Entries.Select(e => e.EnumName));
