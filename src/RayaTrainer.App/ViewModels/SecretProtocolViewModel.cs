...
public sealed class SecretProtocolViewModel : ViewModelBase
{
        ...
        TrainerAppSettings settings)
    {
        ...
        DeleteSecretProtocolPresetCommand = new RelayCommand(DeleteSelectedSecretProtocolPreset, () => _selectedSecretProtocolPreset is not null && !_isQueueRunning());
    }
    ...
    public RelayCommand DeleteSecretProtocolPresetCommand { get; }
    ...
    public string DeleteSecretProtocolPresetHelpText => "删除选中的秘密协议队列预设；游戏内控制台的同名选择会被清空。";
    ...
    private void SaveSecretProtocolPreset()
    {
        try
        {
            ...
            NamedPresetList.Upsert(SecretProtocolPresets, preset);
    ...
    private void DeleteSelectedSecretProtocolPreset()
    {
        if (_selectedSecretProtocolPreset is null) return;
        var name = _selectedSecretProtocolPreset.Name;
        if (!NamedPresetList.RemoveByName(SecretProtocolPresets, name)) return;
        SelectedSecretProtocolPreset = null;
        // 持久化 + 投影同步：Overlay 的同名选择会被 Agent 清空并禁用执行。
        _persistSettings();
        _setStatus($"已删除秘密协议队列预设：{name}");
    }
    ...
    public void RaiseCommandStates()
    {
        ...
        DeleteSecretProtocolPresetCommand.RaiseCanExecuteChanged();
