...
public sealed class HotkeySettingsViewModelTests
{
    ...
    public void ConstructorPopulatesRowsFromFeaturesAndActions()
    {
        var features = new[]
        {
            Feature("Money", "增加玩家战场资金", "Ctrl+F1"),
            Feature("Power", "无限电力", "Ctrl+F2")
        };
        var current = Dict(("Money", "Alt+F1"), ("Power", "Ctrl+F2"));
        var defaults = Dict(("Money", "Ctrl+F1"), ("Power", "Ctrl+F2"));

        var vm = new HotkeySettingsViewModel(features, current, defaults, _ => { });

        Assert.Contains(vm.Rows, r => r.RawName == "Money" && r.CurrentHotkey == "Alt+F1");
    ...
    public void ConflictsAreDetectedWhenTwoRowsShareSameHotkey()
    {
        ...
        var current = Dict(("Money", "Ctrl+F1"), ("Power", "Ctrl+F2"));
        var vm = new HotkeySettingsViewModel(features, current, Dict(("Money", "Ctrl+F1"), ("Power", "Ctrl+F2")), _ => { });

        var money = vm.Rows.Single(r => r.RawName == "Money");
    ...
    public void ClearingHotkeyResolvesConflict()
    {
        ...
        var current = Dict(("Money", "Ctrl+F1"), ("Power", "Ctrl+F1"));
        var vm = new HotkeySettingsViewModel(features, current, Dict(("Money", "Ctrl+F1")), _ => { });
    ...
    public void SaveCommandBlockedWhenConflictExists()
    {
        ...
        var vm = new HotkeySettingsViewModel(features, Dict(("Money", "Ctrl+F1"), ("Power", "Ctrl+F1")), Dict(), _ => { });
    ...
    public void SaveInvokesApplyChangesCallbackWithMergedDictionary()
    {
        ...
        var vm = new HotkeySettingsViewModel(features, Dict(("Money", "Ctrl+F1")), Dict(("Money", "Ctrl+F1")), dict => captured = dict);

        vm.Rows.Single(r => r.RawName == "Money").CurrentHotkey = "Alt+F9";
        ...
        Assert.Equal("Alt+F9", captured!["Money"]);
        // 动作热键行也被序列化（空串表示未分配）。
    ...
    public void ResetAllRestoresDefaultHotkeys()
    {
        ...
        var vm = new HotkeySettingsViewModel(features, Dict(("Money", "Alt+F1")), Dict(("Money", "Ctrl+F1")), _ => { });
        Assert.Equal("Alt+F1", vm.Rows.Single(r => r.RawName == "Money").CurrentHotkey);

        vm.ResetAllCommand.Execute(null);

        Assert.Equal("Ctrl+F1", vm.Rows.Single(r => r.RawName == "Money").CurrentHotkey);
    }

    [Fact]
    public void RowClearCommandSetsCurrentHotkeyToNull()
    {
        var features = new[] { Feature("Money", "增加玩家战场资金", "Ctrl+F1") };
        var vm = new HotkeySettingsViewModel(features, Dict(("Money", "Ctrl+F1")), Dict(("Money", "Ctrl+F1")), _ => { });
        var row = vm.Rows.Single(r => r.RawName == "Money");
    ...
    public void RowClearCommandNotExecutableWhenNoHotkeyAssigned()
    {
        var features = new[] { Feature("Money", "增加玩家战场资金", "Ctrl+F1") };
        // 当前未分配热键 -> 清除命令不可用。
        var vm = new HotkeySettingsViewModel(features, Dict(("Money", (string?)null)), Dict(("Money", "Ctrl+F1")), _ => { });
        var row = vm.Rows.Single(r => r.RawName == "Money");
    ...
    public void SaveEmitsEmptyStringForUnassignedRows()
    {
        var features = new[] { Feature("Money", "增加玩家战场资金", "Ctrl+F1") };
        IReadOnlyDictionary<string, string>? captured = null;
        var vm = new HotkeySettingsViewModel(features, Dict(), Dict(("Money", "Ctrl+F1")), dict => captured = dict);

        // 清空后保存，序列化结果应包含空串值（与配置层「空串=未分配」契约一致）。
        vm.Rows.Single(r => r.RawName == "Money").CurrentHotkey = null;
        vm.SaveCommand.Execute(null);

        Assert.NotNull(captured);
        Assert.Equal(string.Empty, captured!["Money"]);
    }
}
